				...
				for i,b in ipairs(permis_blips) do
					if IsPlayerWantedLevelGreater(GetPlayerIndex(),0) == false and permis.opened == false and  GetDistanceBetweenCoords(b.pos.entering[1],b.pos.entering[2],b.pos.entering[3],GetEntityCoords(LocalPed()),true) > 0 then
						...
						if GetDistanceBetweenCoords(b.pos.entering[1],b.pos.entering[2],b.pos.entering[3],GetEntityCoords(LocalPed()),true) < 4 and IsPedInAnyVehicle(LocalPed(), true) == false then
							ShowInfoPermis("Press ~INPUT_CONTEXT~ to take your ~b~driver's test.", 0)
							inrange = true
						end
					end
				end
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(1,38) and inrangeofpermis then
			blippermis = AddBlipForCoord(344.5361,3418.954,35.40971) --where to go
...
function SpawnPermisCar()
	...
	local vehicle = GetHashKey('blista') --car
...
local spawned_car = CreateVehicle(vehicle, coords, 64.55118,116.613,78.69622, true, false) --where to spawn car
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if onPermis == 1 then
			DrawMissionText2("~b~Drive safely and respect the traffic laws.", 5000) -- Display this when they start the test
		end
		
        local veh = GetVehiclePedIsUsing(GetPlayerPed(-1))
		if IsVehicleDamaged(veh) and onPermis == 1 then
			DrawMissionText2("~r~You failed! Enjoy your walk!", 5000) -- If they crash
